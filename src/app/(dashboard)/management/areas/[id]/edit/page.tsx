...
export default function EditAreaPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  useEffect(() => {
    async function fetchData() {
      const [areaRes, zonesRes] = await Promise.all([
        areasAPI.getById(id),
        zonesAPI.getAll()
      ])
      
      if (areaRes.error) {
        alert('Error loading area: ' + areaRes.error.message)
        router.push('/management/areas')
      } else if (areaRes.data) {
        setFormData({
          code: areaRes.data.code,
          name: areaRes.data.name,
          zoneId: areaRes.data.zone_id,
          isActive: areaRes.data.is_active ? 'true' : 'false',
        })
      }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const { error: err } = await areasAPI.update(id, {
        code: formData.code,
        name: formData.name,
    ...
    <div className="space-y-6">
      <div className="flex items-center gap-4">
        ...
        <div>
          ...
          <p className="text-muted-foreground">Update area information</p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit}>
        <Card>
          <CardHeader>
            ...
            <CardDescription>Update the area details</CardDescription>
          </CardHeader>
